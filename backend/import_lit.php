...
$csvFile = __DIR__ . '/nobel_data/nobel_v5_LIT.csv'; // Adjust path as needed
$rows = [];
...
while (($data = fgetcsv($handle, 1000, ',')) !== false) {
    if (count($data) < 13) { // Now expecting 13 columns
...
// Step 4: Insert Prize Details, Prizes, and Link Laureates
$prize_groups = [];
foreach ($rows as $row) {
...
foreach ($prize_groups as $key => $group) {
    ...
    $row = $group[0]; // Use first row for prize details (assuming same for group)

    // Check if prize already exists
    $stmt = $db->prepare("SELECT id FROM prizes WHERE year = :year AND category = 'literature' AND contrib_sk = :contrib_sk AND contrib_en = :contrib_en");
    $stmt->execute([
        'year' => $year,
    ...
    if (!$prize_id) {
        ...
        $stmt = $db->prepare("INSERT INTO prize_details (language_sk, language_en, genre_sk, genre_en) VALUES (:lang_sk, :lang_en, :genre_sk, :genre_en)");
        $stmt->execute([
            'lang_sk' => $row[9],
            'lang_en' => $row[10],
            'genre_sk' => $row[11],
        ...
        // Insert prize with details_id
        $stmt = $db->prepare("INSERT INTO prizes (year, category, contrib_sk, contrib_en, details_id) VALUES (:year, 'literature', :contrib_sk, :contrib_en, :details_id)");
        $stmt->execute([
            'year' => $year,
            'contrib_sk' => $contrib_sk,
            'contrib_en' => $contrib_en,
...
echo "Literature import completed successfully!\n";