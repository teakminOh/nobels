...
$csvFile = __DIR__ . '/nobel_data/nobel_v5_MED.csv'; // Adjust path as needed
$rows = [];
...
foreach ($prize_groups as $key => $group) {
    ...
    $stmt = $db->prepare("SELECT id FROM prizes WHERE year = :year AND category = 'medicine' AND contrib_sk = :contrib_sk AND contrib_en = :contrib_en");
    $stmt->execute([
        'year' => $year,
    ...
    if (!$prize_id) {
        $stmt = $db->prepare("INSERT INTO prizes (year, category, contrib_sk, contrib_en) VALUES (:year, 'medicine', :contrib_sk, :contrib_en)");
        $stmt->execute([
            'year' => $year,
            'contrib_sk' => $contrib_sk,
            'contrib_en' => $contrib_en,
...
echo "Medicine import completed successfully!\n";